...
class ANOVA:
    ...
    def __post_init__(self):  # TODO: refactor this to be more pythonic
        ...
        full_model_predictors = [
            self.full_model.x_data[:, i] for i in range(self.full_model.predictor_count)
        ]
        reduced_model_predictors = [
            self.reduced_model.x_data[:, i]
            for i in range(self.reduced_model.predictor_count)
        ]
        for index, predictor in enumerate(reduced_model_predictors):
            ...
            if not found:
                raise Exception(
                    f"every column of reduced_model.x_data has to be in full_model.x_data:"
